...
class PipelineConfig(BaseModel): # defenses are defined here
...
class AgentPipeline(BasePipelineElement):
    ...
    def query(
        ...
        logger = Logger().get()
        for element in self.elements:# SystemMessage(basic_elements.py), InitQuery(basic_elements.py), OpenAILLMToolFilter(openai_llm.py), OpenAILLM(openai_llm.py), ToolsExecutionLoop(tool_execution.py)
